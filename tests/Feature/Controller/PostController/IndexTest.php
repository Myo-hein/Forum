...
it('passes posts to the view', function () {
    $expectedPosts = Post::factory(3)->create();

    $expectedPosts->load(['user', 'topic']);

    // Controller orders by latest('id'), so reverse the collection to match
    get(route('posts.index'))
        ->assertHasPaginatedResource('posts', PostResource::collection($expectedPosts->reverse()));
});
